...
#
# 代价（实测，108 chunk 库）：
#   · 精排 API：**零增加**（融合后截断回候选池，池 50→50、批次数 2→2）。
#   · 延迟：其余每一问 +30~60 ms；**冷启动第一问 +527 ms**（≈ jieba 词典初始化
#     0.54 s），且每次文档增删后 BM25 索引缓存失效会再吃一次构建
#     （108 chunk 全量分词 96 ms + 建索引 3 ms）。
#     → 索引按"chunk 总数"做缓存键，所以这两个数都随库大小线性增长。
#
