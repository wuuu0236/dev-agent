...
主要改动：
  1. LLM 客户端换成 langfuse.openai.OpenAI —— Token / 延迟自动上报。
  2. rag_query / generate_answer 用 @observe 装饰，自动捕获输入输出。
  3. metadata（模型、温度等）通过 @observe 的 name 命名区分 span 类型。

对外 API 不变：rag_query(kb_id, query, top_k) -> {answer, sources, query}
"""
...
# 换 client：Token / 延迟会自动出现在 Langfuse
...
def generate_answer(query: str, contexts: list[dict]) -> str:
    """基于检索到的上下文生成答案。Langfuse 自动捕获 query 和 answer。"""
    context_text = "\n\n---\n\n".join(
...
def rag_query(kb_id: str, query: str, top_k: int = TOP_K_RETRIEVE) -> dict:
    """完整的 RAG 查询流程：检索 + 生成。Langfuse 自动捕获输入输出。"""
    # 1. 检索
