...
| P1 | 反馈入口（👍/👎 回标） | 未开始 |
| P2 | 三档回流（缓存作废 / 补文档 / 用例沉淀） | 未开始 |

P0 落地清单（对照 §10 的验收标准，1-5 全部有测试覆盖）：

- `src/answer_log.py` —— 建表（`CREATE TABLE IF NOT EXISTS`，不动旧库）、写入、快照、淘汰、缺口分类
- `src/rag_qa.py` —— 三处写入点：缓存命中 / 检索为空 / 正常生成后；**快照在门控清空 `contexts` 之前抓**（§4.1 的硬约束）
- `src/config.py` —— `ANSWER_LOG_ENABLED` / `_MAX_PER_KB` / `_MAX_HITS` / `_HIT_CHARS`
- `pages/3` —— 解包 5 元组；未命中提示露出最高分；顺带修掉「缓存命中被误报成未命中」
- `pages/4` —— 「🕵️ 问答日志」区块（放在测试集之前），两类待办分开呈现
- `src/database.py` —— `delete_kb` 连带清日志（否则评估面板会统计到孤儿记录）
- `tests/test_answer_log.py` —— 21 条，含「抓取点在门控前」与「信号不被淘汰」两个**已做破坏性验证**的回归测试（把快照挪到门控后 → 失败，0.0 vs 0.28；淘汰不再保护被评价记录 → 失败）
- `tests/conftest.py` —— `stub_langfuse` 夹具（缺 SDK 时注入最小替身），让主链路行为测试
  在任何环境都能跑；**不做 autouse**，否则 `test_smoke_imports` 的「降级验证」会变空转
- 顺带修掉两个既有问题：`_get_ollama_client` 的 `OpenAIClient` 错字（离线路径一用就崩，
  见 `tests/test_rag_backend.py`）／评估面板文案里那条**代码中并不存在**的分界线

> 两处实现与设计的偏差（如实记录）：
>
> ① §4.5 曾考虑"两阶段写"（先 INSERT pending 再 UPDATE）。实现时放弃——Streamlit
> 流式生成在用户关页面时抛 `GeneratorExit`，服务端不可靠捕获，两阶段写会留下永远
> pending 的僵尸行，反而制造"看起来像中断"的假数据。改为生成结束后一次写入，
> 与 `cache_answer` 同处。
>
> ② §4.4 的表格只定义了 `0.20~0.30 → 调检索` 与 `<0.10 → 补文档` 两条，中间那一段
> （0.10~0.20）没有归属。实现时把它并入「差点过阈」，理由见 `answer_log.gap_stats`：
> 两类待办成本不对称，不确定时选更轻的那个。因此**实际只有一条分界线**
> （`0.33 × 阈值` = 默认 0.099），§4.4 表格与之一致，可以直接当契约用。
