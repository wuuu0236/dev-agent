...
- **CI 三道关 + 三条常驻守卫**：GitHub Actions 的 `lint` / `test` 两个 job，测试前先跑一步**依赖自检**——① **静态检查**（pyflakes，要求零告警；曾靠它捞出一个让「本地离线」后端一直 NameError 的错字，云端路径全正常、测试也覆盖不到）② **依赖自检**（逐个 import 核心依赖并打印版本，缺包时把「哪个模块 + 版本 + 异常」直接发成 GitHub annotation）③ **冒烟测试**（22 个测试文件，只跑不调 LLM、不碰密钥的确定性逻辑）。三条常驻守卫专盯「说的和做的不一致」：**首页**出现的每个能力断言都从代码取真实值再比对（首页写"混合检索"就断言 `BM25_WEIGHT > 0`）；**`.env.example`** 必须覆盖 `config` 读到的每个键（漏过必填的 `EMBEDDING_API_KEY`，照配必 401）；**任何模块在没有 API key 的环境里都必须能 import**（曾有模块在导入期就构造客户端，CI 无密钥 → 三个测试文件在收集阶段集体 ImportError，job 红了一个月，本地有 `.env` 完全看不出来）。
