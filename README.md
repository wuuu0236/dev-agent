...
- **CI 双闸门 + 两条「防自相矛盾」的守卫**：GitHub Actions 两道门——① **静态检查**（pyflakes，要求零告警；曾靠它捞出一个让「本地离线」后端一直 NameError 的错字，云端路径全正常、测试也覆盖不到）② **冒烟测试**（21 个测试文件，只跑不调 LLM、不碰密钥的确定性逻辑）。另外两条常驻守卫专门盯「文案和代码有没有脱节」：**首页**出现的每个能力断言都从代码取真实值再比对（首页写"混合检索"就断言 `BM25_WEIGHT > 0`），**`.env.example`** 必须覆盖 `config` 读到的每个键（漏过必填的 `EMBEDDING_API_KEY`，照配必 401）。
...
├── .github/workflows/ci.yml      # CI 双闸门：静态检查（pyflakes 零告警）→ 冒烟测试
